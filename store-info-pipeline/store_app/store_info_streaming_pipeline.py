...
def apply_transformations(lines):
    ...
    d_k = data_by_store | 'Group by store (Apply Transformation)' >> beam.CombinePerKey(
        CombineProductFn())
    return d_k, err
...
def run(argv=None, save_main_session=True):
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        lines = read_data_from_pubsub(pipeline=pipeline,
                                      input_subscription=known_args.input)
        data_by_store, err = apply_transformations(lines)
        data_table = data_by_store | 'Convert to Dict' >> beam.ParDo(
            TupToDictStoreInfo())
        write_to_sink(data_table, known_args.output_table,
                      "Write output to BQ")
