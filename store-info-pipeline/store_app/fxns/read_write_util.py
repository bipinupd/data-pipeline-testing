...
def read_data_from_pubsub(pipeline, input_subscription, label="Read from PubSub"):
  return (pipeline | label >> beam.io.ReadFromPubSub(subscription=input_subscription, timestamp_attribute='ts'))

def write_to_sink(pcollection, bq_table_ref, label):
    _ = (pcollection | label >> beam.io.WriteToBigQuery(bq_table_ref,
