...
def apply_transformation(pcoll, window_size, slide_window, label=""):
     return (pcoll | "ParseMessages" + label >> beam.ParDo(ParseMessages())
       | "ApplySlidingWindow" + label >> WindowInto(window.SlidingWindows(window_size, slide_window))
...
def run(argv=None, save_main_session=True):
  ...
  with Pipeline(options=pipeline_options) as p:
    ...
    transformed_data = apply_transformation(raw_data, int(options.window_size), int(options.slide_window))
    transformed_data | "Print elements" >> Map(lambda ele: print('Ride Status ', ele[0], "Total Passengers: ", ele[1]))
