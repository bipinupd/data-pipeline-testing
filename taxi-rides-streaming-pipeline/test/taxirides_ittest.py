...
class MyDataflow_ITTest(unittest.TestCase):
    ...
    def _inject_messages(self, topic):
      self.list_msg=[
        ...
        {"ride_status": "enroute", "timestamp": "2020-03-27T21:34:51.48098-04:00", "passenger_count": 3},
        ]
      for msg in self.list_msg:
        self.pub_client.publish(topic, f'{json.dumps(msg)}'.encode('utf-8'))
    ...
    def test_apply_transformation(self):
      self._inject_messages(f"projects/{self.project}/topics/{self._testMethodName}_input")
      state_verifier = PipelineStateMatcher(PipelineState.RUNNING)
      pubsub_msg_verifier = PubSubMessageMatcher(
        self.project, self.output_sub.name, expected_msg_len=2, timeout=400, strip_attributes=['id', 'timestamp'])
      extra_opts = {
        'streaming': True,
        'project': self.project,
        'input_subscription': f"projects/{self.project}/subscriptions/{self._testMethodName}_input_sub",
        'output_topic': f"projects/{self.project}/topics/{self._testMethodName}_output",
