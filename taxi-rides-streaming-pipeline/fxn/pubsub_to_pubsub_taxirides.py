...
def read_data_from_pubsub(pipeline, input_subscription):
  return (pipeline | "ReadFromPubSub" >> ReadFromPubSub(subscription=input_subscription, timestamp_attribute='ts'))

def apply_transformations(lines):
  main, error = lines | "ParseMessages" >> ParDo(ParseMessages()).with_outputs(
                    ParseMessages.ERR_REC,
                    main='main')
